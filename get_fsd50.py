...
def get_fsd50k():
    if not os.path.exists("data"):
        os.makedirs("data")
    for f in tqdm(FILES):
        os.system("cd data/ && wget -c %s" % repr(f))
    # TODO: Check hashes
    os.system("cd data/ && zip -s 0 FSD50K.eval_audio.zip --out unsplit.zip && unzip unsplit.zip && rm unsplit.zip")
    os.system("cd data/ && zip -s 0 FSD50K.dev_audio.zip --out unsplit.zip && unzip unsplit.zip && rm unsplit.zip")
